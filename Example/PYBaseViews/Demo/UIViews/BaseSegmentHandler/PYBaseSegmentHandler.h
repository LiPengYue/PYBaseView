 ...
 * 说明：
 * 1. segmentTableView: 用于展示（装着左右横滑的collectionView）最外层的scrollView
 ...
 */
@property (nonatomic,weak) UITableView *segmentTableView;

/**
 * BaseSegmentTagTableHeaderView.y（tabbar 的顶部距离）
 * * 如果segmentTableView的基类是(BaseSegmentTableView) 则可以用 getHeaderFrameWithSection 求出来
 */
@property (nonatomic,assign) CGFloat topSpacing;

/**
 * contentView所在的这一组 所有的cell的高度的和
 * 如果 segmentTableView的基类是(BaseSegmentTableView) 则可以用:
